...
func TestBasic(t *testing.T) {
	...
	e := rn.MakeEnd("end1-99")  //新建终端

	js := &JunkServer{}
	svc := MakeService(js)  //新建服务

	rs := MakeServer() //新建服务器
	rs.AddService(svc)
	rn.AddServer("server99", rs) //服务添加到服务器中

	rn.Connet("end1-99", "server99") //服务器与客户端连接在一起
	rn.Enable("end1-99", true)  //设置可用
